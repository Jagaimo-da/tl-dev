function addChapterToggling() {
    ...
    for (const [index, chapterElem] of Array.from(chapterElems).entries()) {
        ...
        chapterBtn.addEventListener("click", () => {
            ...
            let svg = chapterBtn.querySelector(".chapterBtn__graphic");
...
function addChapterBuoying() {
    ...
    for (const [index, buoyElem] of Array.from(buoyElems).entries()) {
        ...
        const buoyBtn = buoyElem.querySelector(".toc__buoyBtn");
        buoyBtn.addEventListener('click', function () {
...
function testWhack(performTest) {
    const whackChapterElem = document.querySelector("[whackChapter]");
    const whackEpisode = document.querySelector("[whackParentChapter]");
    if (performTest == false) {
        whackChapterElem.remove();
        whackEpisode.remove();
        return;
    }
    const whackChapterTitleElem = whackChapterElem.querySelector(".chapterBtn__chapterTitle");
    const whackTitle = whackChapterTitleElem.textContent;
...
function addAllToggle() {
    let toggleAllElem = document.querySelector(".overviewAside__toggleChapters");
    toggleAllElem.textContent = "Close Chapters";
    toggleAllElem.setAttribute("closeOnClick", "true");
    toggleAllElem.addEventListener("click", () => {
...
function addPossibleTabToggling() {
    const possibleTab_ul = document.querySelector(".possibleTabs");
    ...
    for (const [index, possibleTab_btn] of Array.from(possibleTab_btns).entries()) {
        ...
        possibleTab_btn.addEventListener("click", () => {
